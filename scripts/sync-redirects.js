...
async function getZoneDnsRecords(zoneId) {
  try {
    ...
    throw new Error(
      `Failed to get zone DNS records: ${error.response?.data?.errors?.[0]?.message || error.message}`,
    );
  }
}
...
async function syncRedirects() {
  try {
    ...
    for (const [zoneDomain, rules] of Object.entries(zoneRedirects)) {
      ...
      const existingDnsRecords = await getZoneDnsRecords(zoneId);

      // Create DNS records for subdomains if they don't exist
      const subdomainNames = rules.map(({ source }) => source);
      for (const subdomain of subdomainNames) {
        const existingRecord = existingDnsRecords.find(
          (record) => record.name === subdomain && record.type === "CNAME",
        );

        if (!existingRecord) {
          console.log(`Creating CNAME record for ${subdomain} -> ${zoneDomain}`);
          await createDnsRecord(zoneId, {
            name: subdomain,
            type: "CNAME",
            content: zoneDomain,
            ttl: 1, // Auto TTL
            proxied: true, // Enable Cloudflare proxy for redirects
            comment: "auto-generated-redirect-dns",
          });
        } else {
          console.log(`CNAME record for ${subdomain} already exists`);
        }
      }
      ...
      // Clean up old auto-generated DNS records that are no longer needed
      const autoGeneratedDnsRecords = existingDnsRecords.filter(
        (record) => record.comment === "auto-generated-redirect-dns",
      );

      for (const record of autoGeneratedDnsRecords) {
        if (!subdomainNames.includes(record.name)) {
          console.log(`Removing old CNAME record for ${record.name}`);
